...
def svm_loss_vectorized(W, X, y, reg):
  ...
  num_train = X.shape[0]
  scores = X.mm(W)
  margin = scores - scores[range(num_train),y].view(-1,1) + 1
  ...
  margin[margin<0] = 0
  margin[margin>0] = 1
  margin[range(num_train),y] = -(num_classes-1)
  dW = margin.T.mm(X).T
  dW /= num_train
  dW+=2*reg*W
  #############################################################################
...
def sample_batch(X, y, num_train, batch_size):
  ...
  #########################################################################
  # Replace "pass" statement with your code
  ind = torch.randint(num_train, (batch_size,))
  X_batch = X[ind]
  y_batch = y[ind]
  #########################################################################
  #                       END OF YOUR CODE                                #
...
def predict_linear_classifier(W, X):
  ...
  # _, y_pred2 = X.mm(W).max(dim = 1) # Alternative
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
  return y_pred
...
def softmax_loss_naive(W, X, y, reg):
  ...
  # HINT -> https://ljvmiranda921.github.io/notebook/2017/02/14/softmax-classifier/
  num_classes = W.shape[1]
  num_train = X.shape[0]
  ...
  for i in range(num_train):
    ...
    for j in range(num_classes):
      dW[:, j] += (torch.exp(scores[j]) / torch.exp(scores).sum() - (j==y[i])*1) * X[i, :]
    # for j in range(num_classes):
    #   if j == y[i]:
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  loss = (torch.log(torch.exp(scores).sum(axis=1)) - scores[range(num_train),y]).sum()
  loss /= num_train
  loss += reg * torch.sum(W * W)
