...
def compute_distances_one_loop(x_train, x_test):
  ...
  # Replace "pass" statement with your code
  for i in range(num_train):
    dists[i] = torch.sqrt(torch.sum(((x_train[i] - x_test)**2), dim=(1,2,3)))
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  dists = torch.sqrt((x_train**2).reshape(num_train,-1).sum(dim=(1)).reshape(-1,1) - 2*torch.mm(x_train.reshape(num_train,-1), x_test.reshape(num_test,-1).T) + (x_test**2).reshape(num_test,-1).sum(dim=(1)).reshape(1,-1))
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  #print(dists.topk(k, dim=0, largest=False))
  for i in range(num_test):
    #print(y_train[ind[:,i]].bincount().max(dim=0)[1], "---", y_train[ind[:,i]].bincount(), "---", y_train[ind[:,i]])
    _, y_pred[i] = y_train[ind[:,i]].bincount().max(dim=0)
  ...
  for k in k_choices:
    ...
    for i in range(num_folds):
      ...
      for j in range(num_folds):
        if j!=i:
          train_x.append(x_train_folds[j])
          train_y.append(y_train_folds[j])
      train_x = torch.cat(train_x, dim=0)
      train_y = torch.cat(train_y, dim=0)
      classifier = KnnClassifier(train_x, train_y)
      acc.append(classifier.check_accuracy(test_x, test_y, k))
...
def knn_get_best_k(k_to_accuracies):
  ...
  for i in k_to_accuracies:
    mean = sum(k_to_accuracies[i]) / len(k_to_accuracies[i])
    if mean > max_mean or (mean == max_mean and i < best_k):
      max_mean = mean
      best_k = i
  ##############################################################################
  #                            END OF YOUR CODE                                #
